...
# خطوطِ «برچسب: مقدار»ِ مشخصاتی (نثرِ گفتگویی هیچ‌وقت با این برچسب‌ها + دونقطه شروع نمی‌شود)
_SPEC_LABEL = re.compile(
    r"^\s*(?:قیمت|گارانتی(?: کننده)?(?: در ایران)?|ارسال|زمان ارسال|موجودی|وضعیت"
    r"|لینک|کد|رفرنس|مرجع|شماره مرجع|برند|نام برند|جنس(?: بدنه| بند| بکارگرفته)?"
    r"|سایز|قطر|موتور|نوع موتور|رنگ(?: صفحه)?|طرح بند)\s*[:：]")
...
def strip_product_lines(text):
    ...
    for line in text.splitlines():
        ...
        if _PROD_PREFIX.match(s):  # خطِ کارتِ محصول (با ایموجی شروع می‌شود)
            continue
        if _SPEC_LABEL.match(s):   # خطِ «قیمت: …»/«گارانتی: …»ِ مشخصاتی (نه نثرِ گفتگو)
            continue
        # فقط نشانگرهای قطعیِ خط‌کارت/دکمه — نه کلماتِ پرتکرارِ فارسی که در جملهٔ عادی هم می‌آیند.
